...
bool check_swap(Pos a, Pos b) {
  ...
  // init a temp board
  int temp_gameboard[6][6] = {0};
  for(int i=0; i<6; i++) {
    for(int j=0; j<6; j++) {
      temp_gameboard[i][j] = GEM_NULL;
    }
  }

  if(a.y==b.y) { // a, b 左右排列
    // 處理board[x][2] (中間橫的)
    int left = a.x<b.x?a.x:b.x; // the smaller one
    int border_x = 0;
    for(int i=0; i<6; i++) {
      temp_gameboard[i][2] = gameboard[left-2][a.y].type;
      if(left<BOARD_WIDTH-1) {
        left++;
      }
      else {
        border_x = i+1;
    ...
    if(border_x) {
      for(int k=border_x; k<6; k++) {
        temp_gameboard[k][2] = GEM_NULL;
      }
    }
    // 處理直的兩行
    int temp_y = a.y;
    ...
    for(int i=2; i<4; i++) {
      for(int j=0; j<5; j++) {
        temp_gameboard[i][j] = gameboard[left][temp_y-2].type;
        if(temp_y<BOARD_HEIGHT-1) {
          temp_y++;
        }
        else {
          border_y = j+1;
      ...
      if(border_y) {
        for(int k=border_y; k<6; k++) {
          temp_gameboard[left][k] = GEM_NULL;
        }
      }
      left++;
    }
    // swap(a,b)
    int temp = temp_gameboard[2][2]; //  a
    temp_gameboard[2][2] = temp_gameboard[3][2]; //  a = b
    temp_gameboard[3][2] = temp; //  b = a
  }
  else {  // a, b 上下排列
    // 處理board[2][y] (中間直的)
    int upper = a.y<b.y?a.y:b.y; // the smaller one
    int border_y = 0;
    for(int i=0; i<6; i++) {
      temp_gameboard[2][i] = gameboard[upper-2][a.x].type;
      if(upper<BOARD_WIDTH-1) {
        upper++;
      }
      else {
        border_y = i+1;
    ...
    for(int j=2; j<4; j++) {
      for(int i=0; i<5; i++) {
        temp_gameboard[i][j] = gameboard[upper][temp_x-2].type;
        if(temp_x<BOARD_WIDTH-1) {
          temp_x++;
        }
        else {
          border_x = j+1;
      ...
      if(border_x) {
        for(int k=border_x; k<6; k++) {
          temp_gameboard[k][upper] = GEM_NULL;
        }
      }
      upper++;
    }
    // swap(a,b)
    int temp = temp_gameboard[2][2];
    temp_gameboard[2][2] = temp_gameboard[2][3]; 
    temp_gameboard[2][3] = temp; 
  }
  /*
  現在有:
  一個蝴蝶形狀複製好的 6x6 tempboard, 空位皆為GEM_NULL
  => 仿造check_eliminate!
  */
  for(int i=1; i<5; i++) {
    for(int j=1; j<5; j++) {
      if( // 橫的
         temp_gameboard[i-1][j]==temp_gameboard[i][j]
      && temp_gameboard[i][j]==temp_gameboard[i+1][j]
      // 直的
      || temp_gameboard[i][j-1]==temp_gameboard[i][j]
      && temp_gameboard[i][j]==temp_gameboard[i][j+1]
      // 有寶石存在
      && gameboard[i][j].type!=GEM_NULL) {
        return true;
      }
